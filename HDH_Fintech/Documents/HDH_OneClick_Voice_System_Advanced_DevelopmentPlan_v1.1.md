...
## 4.3 LOT 설정 시스템 (1차 단계)

### 4.3.1 HDH 솔루션 제공 방식 (권장)

**HDH 솔루션에서 최적 LOT 값을 제공**합니다:
- HDH 운영팀이 시장 상황에 따라 최적 LOT 계산
- 사용자에게 LOT 값 제공 (예: "챌린지 6.8 랏, 보험금 0.9 랏")
- 사용자가 음성 또는 수동으로 입력: **"챌린지 육점팔 랏"**
- 프로그램이 자동으로 설정 적용

### 4.3.2 자동 LOT 계산 (옵션)

HDH 솔루션 값이 없을 경우 자동 계산 가능:

**계산 로직**:
- FAIL=-4.1p 지점에서 DD 규정 초과 없이 Fail 발생하도록 LOT 계산
- 입력 파라미터: 계좌 잔고, DD 규정(5%), SL 거리, TP 거리
- 챌린지 LOT이 결정되면 보험금 LOT은 자동 연동 (비율 유지)
...
DD 규정: 5% = $150
SL: -4.1p
→ 챌린지 LOT 자동 계산 = 6.8 lots
→ 보험금 LOT 자동 계산 = 0.9 lots
```
...
```
HDH_AutoTrading_System_v1.1.exe
├─ 1차 챌린지 모드
│   ├─ 챌린지 계좌 + 보험금 계좌 동시 제어
│   ├─ 원클릭 진입/청산
│   ├─ 음성 명령
...
4. "📥 프로그램 다운로드" 버튼 클릭
5. `HDH_AutoTrading_System_v1.1.exe` 다운로드
...
#### Step 3: 설치 및 초기 설정
1. 다운로드한 파일 실행
2. 설치 경로 선택 (기본: `C:\Program Files\HDH_AutoTrading\`)
